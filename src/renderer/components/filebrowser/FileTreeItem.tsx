...
const FileIcon = ({ extension, className }: { extension?: string; className?: string }) => {
  ...
  // Code files
  if (['js', 'jsx', 'ts', 'tsx', 'py', 'rb', 'go', 'rs', 'java', 'kt', 'swift', 'c', 'cpp', 'h', 'hpp', 'cs', 'php'].includes(ext || '')) {
    ...
    <div role="treeitem" aria-selected={isSelected} aria-expanded={isExpanded}>
      ...
      >
        ...
        {gitStatus && (
          <span
            className={cn(
              'ml-1 text-[10px] font-mono font-bold',
              gitStatusColors[gitStatus]
            )}
            title={gitStatus === 'M' ? 'Modified' : gitStatus === 'A' ? 'Added' : gitStatus === 'D' ? 'Deleted' : gitStatus === 'R' ? 'Renamed' : gitStatus === '?' ? 'Untracked' : ''}
          >
            {gitStatus}
          </span>
        )}
